...
`.trim();
  ...
  return moderateContent(content, '投稿', apiKey, baseUrl, language);
}
...
`.trim();
  ...
  return moderateContent(content, '角色申请', apiKey, baseUrl, undefined);
}
    ...
    const response = await fetch(baseUrl, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `你是内容初审员，为管理员提供审核建议。分析用户提交的${contentType}内容，内容语言为${language || '中文'}，评估风险等级。
